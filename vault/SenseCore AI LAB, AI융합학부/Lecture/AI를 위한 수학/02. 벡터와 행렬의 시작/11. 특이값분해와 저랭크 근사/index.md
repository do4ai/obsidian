...
---
09강에서는 어떤 방향이 변환 뒤에도 방향을 유지하는지 고유값으로 읽었고, 10강에서는 벡터를 투영해 가장 가까운 해를 찾았습니다. 11강에서는 한 걸음 더 가서, 복잡한 행렬 전체를 `중요한 방향 몇 개`로 요약하는 방법을 봅니다. 그 도구가 특이값분해이고, 그 요약을 실제 압축으로 바꾸는 방법이 저랭크 근사입니다.
